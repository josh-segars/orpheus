...
| Frontend | Complete | All portal pages shipped — Welcome, Groundwork, Questionnaire (9-question simplified intake post-ORPHEUS-33), LinkedIn upload Step 1 + Step 2, Analysis-in-Progress, Signal Score (redesigned per ORPHEUS-50 — waveform hero + dimension cards + 5-pill band rows + 5-pip sub-dimension ratings), Forward Brief, Cheat Sheet, Login, `/invite/:token`, `/invite/callback`, `/not-invited`, `/advisor/clients` (ORPHEUS-39). React Query + Supabase Auth wiring complete via `frontend/src/lib/`. Dark mode is canonical app-wide as of ORPHEUS-50 (2026-05-29); no light variant. Frontend test infra (vitest + RTL) stood up under ORPHEUS-47, current baseline **8 green** (3 ClientsPage + 5 SignalScorePage). |
