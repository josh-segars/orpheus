...
**Reconciliation operands (ORPHEUS-114, 2026-08-11):** the values the derived ratios are built from are persisted alongside them so the pre-persist identity checks are auditable against stored data — `post_count` (the ORPHEUS-112 denominator), `total_impressions`, `total_engagements`, `net_new_followers`, `followers_weeks_observed`, and `discovery_impressions` (LinkedIn's own DISCOVERY-sheet total, cross-checked against the ENGAGEMENT dailies sum). All Optional; rows scored before 2026-08-11 lack them and are never retro-checked. Six identities run in `backend/scoring/reconciliation.py` and block `stage_scoring` before the scores upsert on failure. Every metric's golden source, unit, denominator, and window lives in `backend/scoring/registry.py` — the single owner of client-facing metric labels for both the sub-dim measured-signal lines (ORPHEUS-117) and the forward-brief prompt block.

**Coverage facts (`forward_brief_data.coverage`, ORPHEUS-114 d):** posts in the scoring window vs the top-50 per-post analytics cap, and per-file date exclusions (`unparseable` / `empty` / `total_rows` for shares/comments/reactions — empty-Date rows are counted via `ZipData.raw_behavioral_row_counts`, recorded at parse time). Rendered as a labelled DATA COVERAGE prompt block so coverage claims in prose trace to inputs; the counts join the ORPHEUS-121 prose-number whitelist.
...
```json
{
  ...
  "forward_brief_data": {
    "quantitative": {
      ...
      "followers_weeks_observed": 52.14,
    ...
    "coverage": {
      ...
      "shares": {"unparseable": 0, "empty": 0, "total_rows": 341},
      "comments": {"unparseable": 70, "empty": 0, "total_rows": 2437},
      "reactions": {"unparseable": 0, "empty": 0, "total_rows": 9644}
    }
  }
}
```

The `post_count`…`discovery_impressions` operands and the `coverage` block are ORPHEUS-114 additions (2026-08-11), all Optional — rows scored earlier lack them.
...
|---|---|---|
| Database schema | Applied | Migration 001 = snapshot of prod public schema (2026-05-11, ORPHEUS-35). On top: 011 (questionnaire spec alignment, ORPHEUS-33), 012 (invitation columns, ORPHEUS-36), 013 (band rename, ORPHEUS-49 — applied to cloud 2026-06-01 under ORPHEUS-25). On top later: 014 (one-clients-row-per-user unique index, ORPHEUS-83), 015 (`jobs.oidc_photo_present`, ORPHEUS-89), 016 (`jobs.data_limited`, ORPHEUS-88 — applied to cloud 2026-07-01, no backfill). On top later: 017 (`public.waitlist` write-only table, ORPHEUS-8) + 018 (waitlist first_name/last_name/interests[], ORPHEUS-8) — committed 2026-07-08, **applied to cloud** (ladder shows 2026-07-02 + a harmless idempotent re-apply 2026-07-08; verified 2026-07-15 during ORPHEUS-104). On top: 020 (consent records — `terms_acceptances` + `jobs.upload_consent_*`, ORPHEUS-126; applied to cloud 2026-08-11 via the Supabase MCP ahead of the deploy). On top: 021 (ORPHEUS-120 data-only backfill — publishes pre-gate advisory reports; applied to cloud 2026-08-12 via the Supabase MCP as `orpheus120_publish_backfill`, BEFORE the gate deploy by design; a one-time backfill, so a fresh DB doesn't need it). On top: 022 (`signup_codes` + `code_redemptions`, ORPHEUS-129 — applied to cloud 2026-08-12 via the Supabase MCP; RLS enabled with no policies, service-role only). On top: 023 (`jobs.prose_gate_degraded` + `jobs.prose_gate_violations`, ORPHEUS-131 — **NOT YET APPLIED TO CLOUD as of 2026-08-17; apply BEFORE the worker deploy**, because the worker writes both columns on every completion and the write fails the job against an un-migrated database). Fresh-DB recipe: `supabase db reset && apply 001 + 011 + 012 + 013 + 014 + 015 + 016 + 017 + 018 + 020 + 022 + 023`. Migrations 007–010 are HISTORICAL — don't apply on top of 001. Migrations 003–006 are baked into 001. |
...
| Ingestion — ZIP parser | Complete | Tested against real data. Handles None keys, datetime formats. Data quality reporting integrated. **Two open gaps found 2026-08-14:** multiline/mis-quoted comments fragment into extra CSV rows whose date field carries comment prose — the source of the "unparseable dates" exclusions (103 of 105 on `b03ca0f5`; share rising 2.5%→4.0% across Andrew's runs, not a code regression) and of the validator's mislabeled message; parser hardening + label fix is **ORPHEUS-138**. And the parser reads exactly 7 CSVs, so `Recommendations_Received.csv` / `Endorsement_Received_Info.csv` present in Complete archives are never ingested — which let the narrative assert recommendations absent for a member who has them (**ORPHEUS-137**; ingest-vs-exclude is Andrew's framework call). |
...
| API routes | Complete | 8 routers (`jobs`, `clients`, `advisor`, `session`, `signup`, `admin`, `account`, `consent`) registered in `backend/main.py`. All client-facing routes role-gated via `get_current_session_roles` (`SessionRoles.is_advisor()` / `.is_client()`); five routes use `get_verified_session` for the neither-role case (`POST /accept-invitation`, `GET /session`, `DELETE /account` — so a partial-failure retry isn't 401'd, ORPHEUS-124 — `POST /consent/terms`, ORPHEUS-126, and `POST /signup/complete`, ORPHEUS-85 — the sign-up completion IS the clients-row creation step). `GET /jobs/{id}` accepts both client-self and advisor-owns-client paths post-ORPHEUS-46. `GET /jobs` (ORPHEUS-81, 2026-06-12) lists the caller's own jobs newest-first with composite band joined from `scores.band` (+ a `data_limited` flag per ORPHEUS-88) — backs the `/reports` page; `POST /jobs` enforces the one-in-flight-run-per-client guard with a 409, plus (ORPHEUS-88/100/101, 2026-07-01) three ordered upload gates that 422 a Basic/corrupt archive (filename prefix + missing-file content backstop) and a stale export (>14 days, from the filename date or the analytics XLSX). As of ORPHEUS-108 (2026-07-15) submission is browser-direct: `POST /jobs/upload-urls` mints signed Storage upload URLs (staging path, guards first), the browser uploads straight to Supabase Storage, and `POST /jobs/from-uploads` runs the same gates (shared `_apply_submission_gates`) against the staged bytes and moves them to the worker's path — the large body never transits the Railway edge. Live validation completed 2026-07-20 (ORPHEUS-86/102 closed) and the legacy multipart `POST /jobs` + `_read_upload` were **deleted 2026-07-27** (`8f1b890`, ORPHEUS-108 closed), so browser-direct is now the only submission path; a client on the pre-108 bundle gets a 405. The shared `_apply_submission_gates` stays as the transport-independent accept/reject policy, pinned directly by `backend/tests/test_submission_gates.py`. `/admin/*` routes use `get_current_admin` (JWT-verify + `ADMIN_EMAILS` allowlist) and bypass RLS via the service-role client — god-mode by design (ORPHEUS-31). |
| Signup/invitation flow | Complete | Resend transactional email + custom backend (`POST /clients/invite`, `POST /accept-invitation`, `POST /clients/{id}/resend-invitation`, all under ORPHEUS-38). Advisor admin UI at `/advisor/clients` (ORPHEUS-39) for managing the roster — list, invite, resend, run-my-own-report. Cloud Supabase + prod LinkedIn OIDC wired under ORPHEUS-25 (2026-06-01). **ORPHEUS-44 closed 2026-06-02 part 2** — full live e2e validated end-to-end against cloud: invite acceptance, questionnaire save, Groundwork → Analysis → Signal Score / Forward Brief / Cheat Sheet placeholder, advisor uncloak (ORPHEUS-46), and admin narrative edit with `edited_text` overlay surfacing to the client on next poll (ORPHEUS-31 + ORPHEUS-59). Two in-flight bugs shipped same session: ORPHEUS-59 (handler payload schema reconcile) + ORPHEUS-61 (advisors.email → auth.users lookup). ORPHEUS-60 filed as the only deferred follow-up (narrative agent emits structured cheat_sheet). **Report-completion feedback email shipped under ORPHEUS-98 (2026-06-24)** — a thank-you + beta-survey CTA (`format_report_ready_email`) sent once per client when their report becomes viewable: self-serve at pipeline completion (worker), advisory at publication (admin narrative PATCH when the last draft flips to published); idempotency + once-per-client ride `reports.published_at`; optional `BETA_SURVEY_URL` env on both Railway services; consent footer deferred for the closed beta. **Not yet observed delivering in production** — ORPHEUS-119 (2026-07-27) traced the absence to the ORPHEUS-118 DNS outage rather than a send-path bug (both triggers read correctly against live data), and left live verification pending the next first-time completion under an `is_individual = true` advisor. The advisory publication trigger has never fired at all, because **ORPHEUS-120** found the draft gate didn't hold on the read path: `_build_result_payload` read narratives service-role without a `status` filter, so clients saw draft narratives at pipeline completion — which is also why 14 advisory-roster clients read their reports while `reports.published_at` stayed NULL and no feedback ask was ever sent. **The gate is enforced as of 2026-08-11 part 4** (`c283ddb`, deployed): `GET /jobs/{id}` nulls the payload for a client viewing a complete advisory job with `published_at IS NULL` (job-level signal, `in_review` wire state, "advisor is reviewing" surfaces; advisors keep seeing drafts), and migration 021 published the 14 pre-gate clients' reports so nothing vanished — their feedback asks stay unsent by design. ORPHEUS-120 sits In Progress awaiting Andrew's first real review-then-release as live acceptance; with the gate enforced, publication is now a genuine readiness moment, resolving the trigger-timing question in favor of the current trigger. **Resend messaging shipped under ORPHEUS-93 (2026-07-13)** — resend rotates the token (kills the earlier link), so the resend email now says it replaces any earlier invitation, the accept-invitation 401 points at the recovery path, and the advisor's Resend button requires an inline confirm. ORPHEUS-94 (email-mismatch reads as an error) remains open. **Self-serve sign-up shipped 2026-08-12 and closed 2026-08-13 (ORPHEUS-85 + 129)** — validated by the first real code-gated sign-up (clients row `176450e3` under house advisor `6b9922b9`, code `ORPH-Z32A-K7VA` redeemed 113 ms later); that client has not submitted an archive, so ORPHEUS-121 / 114 / 119 still wait on a completion, and the ORPHEUS-85 Decision Log entry is owed now that sign-up is live — `POST /signup/complete` (idempotent get-or-create under the dedicated house advisor row or a code's routing override, gated by admin-generated codes in `public.signup_codes`), public `/signup` + `/signup/callback`, `/admin` Codes section (`GET/POST /admin/codes` + disable/enable PATCH). Invitations remain the advisory path; sign-up is the self-serve one. |
...
| Forward Brief computation | Complete | Computed in scoring stage (unchanged by ORPHEUS-68). XLSX feeds reach/audience metrics; ZIP feeds qualitative flags. As of ORPHEUS-67/68/69 (2026-06-10) the data no longer feeds a standalone narrative — it grounds the per-dimension forward-looking guidance and renders as the Signal Score page's structured metrics block. **`avg_impressions_per_post` corrected under ORPHEUS-112 (2026-07-27, closed 08-03)** — it divided total impressions by days-with-impressions in the ENGAGEMENT sheet (one row per calendar *day*, no post count in it), which for any active member runs to the export window's length, so the metric was impressions per *day* under a per-post label (875.4 vs. a true 2,852.8 on `b902bd06`). Now divides by posts published inside the scoring window, counted from `zip_data.shares` and sharing the `post_dates` computation with the posting-gap metrics. **An approximation, not an identity** — the numerator covers impressions earned in-window by all live content including pre-window posts (bounded by how fast impressions decay), and the two sides come from different windows (analytics export vs. the ORPHEUS-91 ZIP-activity anchor; 3 days at the head and 2 at the tail on that dataset). Documented on the model field and at the call site; ORPHEUS-114's reconciliation tolerance must absorb window-edge publishing, not just rounding. **The ORPHEUS-114 reconciliation gate shipped 2026-08-11 part 4** (`7dcface`): operands persist alongside the derived metrics, six identities (incl. the DISCOVERY-vs-ENGAGEMENT cross-source check and a members_reached range check — never a sum-of-dailies) run in `stage_scoring` before the scores upsert and fail the job on violation; the metric registry (`backend/scoring/registry.py`) owns every label/unit/denominator; coverage facts (top-50 cap, per-file date exclusions) persist on `forward_brief_data.coverage`. Acceptance ran against production data (job `0007607e` — all six identities green, every golden fixture reproduced). **Closed 2026-08-14** on Andrew's 2026-08-13 job `b03ca0f5` — the first worker-run job under the gate: operands and the coverage block both persisted, and all six identities re-verified independently from the stored row (impressions/post 2,820.2 x 125 posts vs 352,526 total, |Δ|=1; DISCOVERY vs ENGAGEMENT at Δ=0). The decisive evidence is structural rather than a log line — `stage_scoring` raises before the scores upsert, so a persisted `scores` row *is* the green result. |
...
| Narrative generation | Complete | `agents/narrative.py` — receives scored dimensions + forward brief data + quality report + 9-question intake (post-ORPHEUS-34 prompt rewrite). Single Claude call (`max_tokens` 8192) emits 4 dimension sections — each a combined messaging paragraph (up to ~400w, interpretation + forward-looking guidance; length floors dropped across all narrative layers under ORPHEUS-66, 2026-06-10) plus an always-visible 1–2 sentence `summary` teaser (ORPHEUS-68, 2026-06-10; the standalone forward_brief section is retired) — AND 13 per-sub-dim narrative payloads (ORPHEUS-21, score-keyed conditional structure) AND the structured cheat_sheet (ORPHEUS-60). Return type is `NarrativeResult` NamedTuple (`sections` + `summaries` + `sub_dimensions` + `cheat_sheet`); worker's Stage 4b merges sub-dim payloads and dim summaries back into `scoring_output` and re-persists `scores.dimensions` JSONB. Live cloud validation of the ORPHEUS-68 reaxis tracked under ORPHEUS-73. Voice: platform default is **direct second person** (`second_person_direct`) as of ORPHEUS-77 (2026-06-10) — revises the original advisory = third-person split; `third_person_neutral` stays selectable per-advisor via `advisors.narrative_config`. **Metric handling hardened under ORPHEUS-117 + 113 (2026-07-27, closed 08-03).** Quantitative sub-dim raw values now reach the prompt through `QUANTITATIVE_METRIC_LABELS` carrying unit + measurement window + gloss (windows interpolated from `scoring.config`), replacing a bare `raw value: N` the agent had been quoting verbatim to clients as invented units ("6,316 activity units"); Engagement Quality Score's value is withheld entirely as a unitless internal index, and rendering **fails closed** so an unregistered raw value is omitted rather than passed through unlabeled. Cheat-sheet milestone *values* are computed in code (`build_milestone_targets` — four baseline-derived entries plus starter constants, magnitude-scaled rounding, `MILESTONE_GROWTH_FACTORS` / `MILESTONE_HORIZON_WEEKS` / `FOLLOWER_TREND_STRETCH` all PROVISIONAL and Andrew's to tune) and the agent may only phrase the label, which is digit-banned; the parser enforces value-for-value equality and retries on drift. Followers is trend-derived rather than a multiple of the count (`861d581`) so a target can't fall below the growth rate the report prints alongside it. **The ORPHEUS-121 prose-number gate shipped 2026-08-11 part 4** (`d8b7876`): every numeric token in client-facing prose must match a whitelisted measured value (built from the ORPHEUS-114 registry, coverage counts, sub-dim raw values, computed milestones) or a structural allowance; violations reject and retry with the offending tokens fed back into the prompt; `PROSE_NUMBER_GATE` env kill switch (block/log/off); the "2,394"/"331" fabrications are pinned as regressions. **Closed 2026-08-14 [Josh]** on Andrew's 2026-08-13 job `b03ca0f5`: the gate blocked a live fabrication ("2,549", matching no input — nearest real values are 2,611 comment rows and the ORPHEUS-117-*suppressed* 2,555.25 quality index, so it was minted from nothing) and the delivered prose quotes only figures it was handed. **ORPHEUS-131 closed that cost out in code 2026-08-17** (`d2263fd` / `f802d92` / `13f7f6c` / `b434a96`): on b03ca0f5 two of the worker's three attempts went to gate rejections (~9 generations, six minutes) and the job passed on attempt 3 of 3, so a report could fail on prose alone. The gate now rejects and retries the non-final attempts exactly as before and DEGRADES on the last one — the narrative is served with the violation marked on `NarrativeResult.prose_gate_degraded` / `prose_gate_violations`, persisted to the job row by migration 023 and chipped in /admin as "unverified figures". `log` mode marks the same way (a global override that hid its own violations was the problem); `off` never marks. Parse failures still raise. A degraded report still publishes [Josh, 2026-08-17] — the marker is the review surface, not a hold, because self-serve is the path with nobody watching and a parked report is a report that never arrives. `scripts/regenerate_report.py` takes the opposite posture: its `verify()` now FAILS on the marker, since it overwrites a report the client has already read and never touches the job row that would flag it — it had been protected only by the exception the degrade removes. Deliberately left open: stages 2 and 3 still re-run on a narrative-only failure (**ORPHEUS-140**). ORPHEUS-115 (mislabeling a real value) remains open — the registry's canonical labels are now available as its substrate. **The claims layer is the open front (2026-08-14 review reconciliation):** the Foundational Review's Spec 2 guardrails (no behavior→reach/visibility claims, no effect sizes, signal-legibility/human-reader registers only) were never implemented in the prompt — Andrew's v3 sweep of `b03ca0f5` found ~20 Ruling-2 breaches across every generated surface while every *number* verified exactly. Filed as **ORPHEUS-134** (guardrails), **135** (footer), **136** (Spec-1 rubric sentence), **137** (absence-assertion ban + recommendations ingest-vs-exclude); milestone retargeting under Ruling 2 (posts/week = behavior target, survives; impressions/followers/rate = quantified outcome targets) is a team-discussion decision, agenda at repo root. **First pass shipped 2026-08-17** (`73aead8` + `0ebf031`): a `## Claims layer` section in `SYSTEM_PROMPT_TEMPLATE` carrying Spec 2's three additions (no behavior→reach/visibility/distribution claims and no threshold assertions, with the hedged forms named as the same claim; signal-legibility or human-reader framing with human-reader preferred; mechanism claims closed to three documented statements and never quantified) plus 137's absence-assertion ban and the recommendations/endorsements/skill-ordering exclusion. One insertion covers all surfaces because generation is a single Claude call. The prompt's own cheat-sheet example previously modelled the banned action ("Target: 2 new recommendations in 30 days") and was replaced, as was the `new_recommendations` milestone starter — a code-owned value no prompt rule could have reached. **Acceptance harness: `scripts/claims_sweep.py`** (`b63d92e`, read-only, source-level-asserted no-write) — twelve breach families, a detector self-test that aborts the run if a HIGH family stops catching its own v3 exemplar, a control pass over stored delivered text to prove the detector fires on real prose, and a full per-run text dump which is the actual acceptance artifact. **Sweep result 2026-08-17: 134 needs a second pass.** All four dimension narratives and all thirteen sub-dim slots clean across five generations, and 137's families scored 0 against 10 in the control — but every run produced an attribution instruction in the cheat sheet's rhythm block, a family the first pass banned *making* while leaving *delegating to the member* untouched. Flag counts under-reported it (3 of 5 runs) and the dump read corrected it to 5 of 5. **ORPHEUS-142 (high) — surfaced by that sweep, fixed 2026-08-18** (`16371ff`): `build_number_whitelist` never received the profile text ORPHEUS-96 added to the prompt, so all five runs degraded on correct citations of the member's own dollar figures; the whitelist now admits every numeric token in the rendered profile excerpt (citable == visible-to-the-agent; figures beyond the truncation caps stay gated; a fabricated-aggregate control is a permanent unit test), the acceptance re-run went 0-of-5 degrades with the gate re-validated live mid-run, and 142 + 131 hold In Progress pending the worker deploy (migration 023 first) plus one real job to observe the marker and /admin chip live. **134's second and third passes shipped 2026-08-18** (`2a30194`/`58ba3f0`/`d133239`): Claims rule 7 (a review item may ask the member to observe, never to attribute outcomes), rule 3 extended to ranked leverage, inline Priorities/Rhythm reinforcement, detector at 99 self-testing patterns — attribution homework eliminated from the cheat sheet across ten sweep runs, real breach density ~20 → 1–3 per run, plateaued. **Detect-and-ban paused as the enforcement strategy [Josh, 2026-08-18]** — the residual is structural (reach-titled priorities restate the outcome milestones; thresholds mutate registers), and the alternatives (judge-and-rewrite pass, structured cheat sheet, positive-register prompt = the instrument-vs-coach voice call) are item 8 of the claims-layer discussion agenda — prep docs in Drive as of 2026-08-19: a line-level prompt-vs-claims conflict map and the verbatim prompt set. **Deployed to production 2026-08-26:** migration 023 applied, worker + API rebuilt (anthropic pinned to 0.125.0 after SDK 1.0.0 removed the `temperature` kwarg and an unpinned build failed every job at the rubric stage — ORPHEUS-143, closed same day), and the first real job (`2ebd4432`) ran clean on the first attempt with the degrade marker written — **ORPHEUS-131 + 142 closed**. The same day's first real publish exercised the ORPHEUS-120 draft gate end-to-end (client correctly blocked pre-publish; five per-narrative flips from /admin; `published_at` stamped; report-ready email fired) — 120 closed, with the publish-path UX failures filed as ORPHEUS-144. |
...
| Frontend | Complete | All portal pages shipped — Welcome, Groundwork, Questionnaire (9-question simplified intake post-ORPHEUS-33), LinkedIn upload Step 1 + Step 2, Analysis-in-Progress, Signal Score (redesigned per ORPHEUS-50 + restructured per ORPHEUS-51 — contained waveform hero with band-keyed assets + dimension cards + 5-pill band rows + 5-pip sub-dimension ratings), Cheat Sheet, Login, `/invite/:token`, `/invite/callback`, `/not-invited`, `/signup` + `/signup/callback` (ORPHEUS-85/129 — beta-code + consent + LinkedIn OIDC, inline code re-prompt, `?code=` prefill), `/advisor/clients` (ORPHEUS-39), `/admin` (ORPHEUS-31 — email-allowlisted stopgap; clients + jobs + inline narrative editor, plus a read-only waitlist section with signup-count/interests header stats as of ORPHEUS-104, 2026-07-15, and a sign-up Codes section — mint/disable, redemption counts, shareable links, plus a per-code roster (`GET /admin/codes/{code_id}/redemptions`) listing every client who signed up through that code, newest first, which is the proto-cohort view a group or business code implies; a client deleted mid-request is skipped rather than 500'd, since CASCADE removes the redemption by the next fetch — as of ORPHEUS-129, 2026-08-12). React Query + Supabase Auth wiring complete via `frontend/src/lib/`. Dark mode is canonical app-wide as of ORPHEUS-50 (2026-05-29); no light variant. PortalNav identity cluster ("Prepared for / [Name]" + logout, role-aware name source) shipped app-wide under ORPHEUS-52 (2026-05-30), then folded into the account dropdown with "Logged in as" reframe under ORPHEUS-71 (2026-06-08). App-wide palette + chrome refresh under ORPHEUS-70 (2026-06-08) — brighter green/cyan accents, magenta-anchored pip spectrum, deeper `#00080e` page bg, pure-white text, taller nav, and active band pills that take their band's pip color. **Forward Brief page retired under ORPHEUS-69 (2026-06-10)** — dimension cards carry summary + read-more toggle, `forward_brief_data` renders as the metrics block at the bottom of the Signal Score page, flow is Signal Score → Cheat Sheet. Cheat Sheet subtitle resolves the report subject via the ORPHEUS-71 hero pattern as of ORPHEUS-74 (2026-06-10). Client-facing copy renamed under ORPHEUS-76 (2026-06-11): "Signal Score" → "report", "Cheat Sheet" → "Quick Reference Card" (user-facing strings only; routes + internal names unchanged); same pass fixed the login wordmark positioning context and added Figma-spec sub-dim expand carets. **Multi-report support shipped under ORPHEUS-81 (2026-06-12)** — new `/reports` list page is the client landing surface (one row per job: date, band chip in spectrum pip colors, status; in-flight rows link to Analysis), "Run a New Report" re-enters Groundwork (hidden while a job is in flight; backend 409 is authoritative), smart redirect routes any job history to the list, nav "View My Reports" → `/reports`. ORPHEUS-78 verbiage pass same session: "View My Quick Reference Card", shortened dimension display names (Profile Clarity / Signal Strength / Signal Quality / Alignment via `DIM_DISPLAY_NAMES`; internal names canonical upstream), Welcome card retitled. A data-limited report banner + "Limited data" chips (reports list / advisor roster / admin) shipped under ORPHEUS-88 (2026-07-01); ORPHEUS-106 (2026-07-09) made the underlying `parse_failure` classification proportional (a `QualityIssue.total_rows`-driven >10%-of-source threshold in `DataQualityReport.data_limitation_issues()`), so a small unparseable-date tail no longer trips the banner/chip. **Public marketing landing page + waitlist shipped under ORPHEUS-8 (2026-07-08)** — an `isMarketingHost()` branch in `App.tsx` serves a no-auth `LandingPage` at `/` on the www/apex host (portal stays on `app.*`; `/site` previews on localhost), with browser-direct waitlist capture to a write-only `public.waitlist` table (migrations 017/018). Groundwork submission is browser-direct to Supabase Storage as of ORPHEUS-108 (2026-07-15) — `useCreateJob` mints signed upload URLs, uploads via `uploadToSignedUrl`, then submits `POST /jobs/from-uploads`; a failed transfer surfaces as the ORPHEUS-86 `NetworkError` guidance, and as of ORPHEUS-109 (2026-07-20) both files are rebuilt with canonical MIME before upload (Windows reports .zip as `application/x-zip-compressed`, which the Storage bucket allowlist rejected) and a Storage 4xx surfaces the service's own reason via `UploadRejectedError` instead of the connection copy. The landing page took Andrew's revised copy under ORPHEUS-122 (2026-07-30) — Pricing became Access (Beta Assessment / Live Cohorts), a founder-credibility block sits above the interest form, and the "outcomes will vary" expectations paragraph is dropped; the waitlist interest *labels* renamed while the stored `beta_access` / `live_workshop` values stayed canonical. **In-app browsers are blocked from the LinkedIn OIDC hop as of ORPHEUS-130 (2026-08-13)** — `/login`, `/signup` and `/invite/:token` detect an app's built-in browser (LinkedIn, Instagram, Facebook, Messenger, Slack, X, generic Android WebView) via `lib/inAppBrowser.ts` and replace the sign-in action with `InAppBrowserNotice`: per-platform open-in-your-browser instructions plus a copy-link that preserves the full URL so an invitation token or `?code=` prefill survives. The LinkedIn app intercepts its own OAuth domain and drops the user on their feed, so the hop cannot complete there at all — four `/authorize` redirects produced zero `/callback` hits for the reporting tester. `/invite/:token` suppresses its mount-time redirect rather than merely restyling. A quiet "continue here anyway" escape hatch backs the hard block because the detection is a UA heuristic. Frontend test infra (vitest + RTL) stood up under ORPHEUS-47, current baseline **186 green** (as of ORPHEUS-130, 2026-08-13; measured on Josh's machine). |
| Legal & consent surface | Complete (beta) | `/privacy` + `/terms` published 2026-08-11 on both hosts from versioned markdown in `frontend/src/content/legal/` (ORPHEUS-125); effective date must match `CURRENT_*_VERSION` in `lib/consent.ts` + `backend/consent_versions.py` — bump all three together. ToS/Privacy acceptance checkbox on /login recorded to `terms_acceptances`; per-upload consent gates `POST /jobs/from-uploads` (ORPHEUS-126, Route A — **closed 2026-08-14** on job `b03ca0f5`, whose `upload_consent_at` lands 133 ms *before* the job row, the enforced ordering visible in data). **The account-level leg has an open coverage gap: `/invite/:token` never shows either document** — only `/login` and `/signup` carry the checkbox and `/accept-invitation` records nothing, so every invited client (Andrew's whole roster) reaches a report with no `terms_acceptances` row. Carved out as **ORPHEUS-132**, whose gate **shipped 2026-08-14** (`8bbc8f0`): `/invite/:token` now hard-gates on the same unticked-by-default checkbox as `/login`/`/signup`, with the version pair riding the OAuth redirect via the shared `withAcceptanceParams` helper. Forward-only — the roster self-heals at next `/login`, and the retrospective question is answered by the beta-end purge decision [Josh, 2026-08-14] (ORPHEUS-133), so the Tim routing is withdrawn. In Progress pending the first real invitation acceptance. Self-service account deletion at `/account` (ORPHEUS-124, live-validated). Manual fulfilment of every §12 right is documented in `Runbook_Data_Subject_Requests_2026-08-11.md` (ORPHEUS-127, closed 2026-08-11) — production-tested SQL for erasure/access/portability, the `clients.user_id` SET-NULL trap, and the out-of-repo DSR log (Shared Canon / 06_Operations). |
