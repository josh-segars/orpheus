...
3. **`git add -A` is unsafe in this repo — do not follow the `orpheus-session-wrap` skill's step 7 literally.** Ten untracked-by-intent paths are **not** in `.gitignore` (only the compliance drafts are): `.claude/`, `outputs/`, `Draft_Cohort_Rubric_*`, `Draft_Unit_Narrative_Questionnaire_*`, `Scoping_B2B_Cohort_Assessment_*`, `Scoping_Free_Tier_And_Premium_*`, `Survey_Closed_Beta_Feedback_*`, `create_beta_survey_form.gs`, both `rubric_consistency_results_*.json`, the ORPHEUS-90 brief. `git add -A` sweeps all of them — the exact accident the `.gitignore` comment memorialises from 2026-05-29. Stage explicitly.
...
17. **Untracked-by-intent files** — do not `git add`: the ten paths in caveat 3.
